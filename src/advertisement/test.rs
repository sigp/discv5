...
async fn ticket_wait_time_no_wait_time() {
    ...
    let key = CombinedKey::generate_secp256k1();
    let enr = EnrBuilder::new("v4").ip(ip).udp4(port).build(&key).unwrap();
    let mut ads = Ads::new(Duration::from_secs(1), 10, 50, 100, 100);
    let topic = Topic::new(std::str::from_utf8(&[1u8; 32]).unwrap()).hash();
...
async fn ticket_wait_time_duration() {
    ...
    let mut ads = Ads::new(Duration::from_secs(3), 1, 3, 100, 100);
...
async fn ticket_wait_time_full_table() {
    ...
    let mut ads = Ads::new(Duration::from_secs(3), 2, 3, 100, 100);
...
async fn ticket_wait_time_full_topic() {
    ...
    let mut ads = Ads::new(Duration::from_secs(3), 2, 4, 100, 100);
...
async fn ticket_wait_time_full_subnet() {
    ...
    let mut ads = Ads::new(Duration::from_secs(2), 2, 4, 2, 1);
