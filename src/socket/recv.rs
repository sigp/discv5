...
pub struct RecvHandlerConfig<T: Executor> {
    ...
    pub handler: tokio::mpsc::Sender<InboundPacket>,
}
...
impl RecvHandler {
    ...
    pub(crate) fn spawn<T: Executor>(config: RecvHandlerConfig) -> tokio::oneshot::Sender {
    ...
    async fn handle_inbound(&mut self, src: SocketAddr, length: usize) {
        ...
        let packet = match Packet::decode(&self.recv_buffer[..length], &self.whoareyou_magic) {
            Ok(p) => p
            Err(e) => { debug!("Packet decoding failed: {}", e), // could not decode the packet, drop it
                       return;
            }
        }
